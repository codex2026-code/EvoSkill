...
OpenAI SDK path via function tools executed locally.
"""
...
class OpenAILocalToolRuntime:
    ...
    _last_bash_output: dict[str, Any] = field(default_factory=dict)
    ...
    def get_openai_tools(self) -> list[dict[str, Any]]:
        """Return tool schemas limited to currently allowed tools."""
        all_defs: dict[str, dict[str, Any]] = {
            "Read": {
                "type": "function",
                "function": {
                    "name": "Read",
                    "description": "Read UTF-8 text file content from allowed paths.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "path": {"type": "string"},
            ...
            "Write": {
                ...
                "function": {
                    "name": "Write",
                    "description": "Write UTF-8 text to a file in allowed paths.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "path": {"type": "string"},
            ...
            "Glob": {
                ...
                "function": {
                    "name": "Glob",
                    "description": "Find files by glob under an allowed base directory.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "pattern": {"type": "string"},
            ...
            "Grep": {
                ...
                "function": {
                    "name": "Grep",
                    "description": "Search regex pattern recursively in text files.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "pattern": {"type": "string"},
            ...
            "Bash": {
                ...
                "function": {
                    "name": "Bash",
                    "description": "Run shell command locally under configured cwd.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "command": {"type": "string"},
            ...
            "BashOutput": {
                ...
                "function": {
                    "name": "BashOutput",
                    "description": "Return output from the most recent Bash command.",
            ...
            "TodoWrite": {
                ...
                "function": {
                    "name": "TodoWrite",
                    "description": "Store simple todo state for current run.",
                    "parameters": {
                        "type": "object",
                        "properties": {
            ...
            "Skill": {
                ...
                "function": {
                    "name": "Skill",
                    "description": "Load local SKILL.md by skill name from common skill dirs.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "name": {"type": "string"},
            ...
            "WebFetch": {
                ...
                "function": {
                    "name": "WebFetch",
                    "description": "Fetch URL text via HTTP GET.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "url": {"type": "string"},
            ...
            "WebSearch": {
                ...
                "function": {
                    "name": "WebSearch",
                    "description": "Lightweight web search returning top links.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "query": {"type": "string"},
    ...
    async def execute(self, tool_name: str, raw_args: str) -> str:
        ...
        try:
            ...
            if tool_name == "BashOutput":
                return self._tool_bash_output()
            if tool_name == "TodoWrite":
                return self._tool_todo_write(args)
            if tool_name == "Skill":
                return self._tool_skill(args)
            if tool_name == "WebFetch":
                return self._tool_webfetch(args)
            if tool_name == "WebSearch":
                return self._tool_websearch(args)
    ...
    def _tool_read(self, args: dict[str, Any]) -> str:
        ...
        selected = lines[start_line - 1 :] if end_line is None else lines[start_line - 1 : int(end_line)]
        return _safe_json_dump(
            {"ok": True, "path": str(path), "content": "\n".join(selected), "line_count": len(selected)}
        )
    ...
    def _tool_glob(self, args: dict[str, Any]) -> str:
        ...
        matches = sorted(str(p) for p in iterator)
    ...
    def _tool_grep(self, args: dict[str, Any]) -> str:
        ...
        rx = re.compile(pattern, flags=re.IGNORECASE if ignore_case else 0)
        out: list[dict[str, Any]] = []

        files = [root] if root.is_file() else list(root.rglob(file_glob))
        for f in files:
            if not f.is_file():
                continue
            try:
    ...
    async def _tool_bash(self, args: dict[str, Any]) -> str:
        ...
        self._last_bash_output = payload
        return _safe_json_dump(payload)

    def _tool_bash_output(self) -> str:
        if not self._last_bash_output:
            return _safe_json_dump({"ok": False, "error": "No previous Bash output available"})
        return _safe_json_dump(self._last_bash_output)
    ...
    def _tool_skill(self, args: dict[str, Any]) -> str:
        ...
        for skill_file in candidates:
            skill_dir_name = skill_file.parent.name.lower()
            if lowered in skill_dir_name or skill_dir_name in lowered:
                content = skill_file.read_text(encoding="utf-8")
                return _safe_json_dump({"ok": True, "path": str(skill_file), "content": content})
        return _safe_json_dump({"ok": False, "error": f"Skill not found: {name}"})

    def _tool_webfetch(self, args: dict[str, Any]) -> str:
        ...
        request = urllib.request.Request(
            url,
            headers={"User-Agent": "EvoSkill-OpenAI-LocalToolRuntime/1.0"},
        )
    ...
    def _tool_websearch(self, args: dict[str, Any]) -> str:
        ...
        return _safe_json_dump({"ok": True, "query": query, "results": results})
